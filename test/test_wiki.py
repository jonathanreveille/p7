...
def test_mediawiki_api_response(monkeypatch):
    ...
    assert response.json(
        IMITATION_RESPONSE_MEDIAWIKI) == IMITATION_RESPONSE_MEDIAWIKI
...
def test_get_json():
    ...
    mediawiki.get_json()
    assert isinstance(mediawiki.search_json, dict)
...
def test_get_title_from_json_object(monkeypatch):
    ...
    assert place.get_title(
        IMITATION_RESPONSE_MEDIAWIKI_2) == 'Academy of Art University'
...
def test_get_extract_from_json_object_with_pageid(monkeypatch):
    ...
    class MockGetSummary:
        ...
        def get_summary(self):
            page = 6422233
            return IMITATION_RESPONSE_MEDIAWIKI_3["query"]["pages"][str(
                page)]["extract"]
    ...
    assert summary.get_summary(
        IMITATION_RESPONSE_MEDIAWIKI_3) == "L’Academy of Art University (autrefois Academy of Art College), est une université appartenant au Stephens Institute, fondée à San Francisco en Californie en 1929 par le peintre Richard S. Stephens."
